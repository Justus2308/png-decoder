...
func makeIHDR(w, h, bpp int, alpha, interlaced bool) []byte {
	ihdr := utils.U32toBBig(uint32(13)) // data field length
	ihdr = append(ihdr, global.IHDR...) // chunk type field
	ihdr = append(ihdr, utils.U32toBBig(uint32(w))...) // width in 4 bits
	ihdr = append(ihdr, utils.U32toBBig(uint32(h))...) // height in 4 bits
	...
	ihdr = append(ihdr, utils.U32toBBig(crc32.ChecksumIEEE(ihdr[4:]))...) // crc32 checksum
	return ihdr
}

func makePLTE(palette []byte) []byte {
	plte := utils.U32toBBig(uint32(len(palette))) // data field length
	plte = append(plte, global.PLTE...) // chunk type field
	plte = append(plte, palette...) // data field
	plte = append(plte, utils.U32toBBig(crc32.ChecksumIEEE(plte[4:]))...) // crc32 checksum
	return plte
}

func makeIDAT(data []byte) []byte {
	idat := utils.U32toBBig(uint32(len(data))) // data field length
	idat = append(idat, global.IDAT...) // chunk type field
	idat = append(idat, data...) // data field
	idat = append(idat, utils.U32toBBig(crc32.ChecksumIEEE(idat[4:]))...) // crc32 checksum
	return idat
}
...
func makeIEND() []byte {
	...
	iend = append(iend, utils.U32toBBig(crc32.ChecksumIEEE(iend[4:]))...) // crc32 checksum
	return iend
}